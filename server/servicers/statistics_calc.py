        ...
        try:
            for param in default_config:
                try:
                    ...
                    process_config[param] = req_param if req_param else default_config[param]
                except AttributeError:
                    process_config[param] = default_config[param]
                    pass

            document = np.genfromtxt(BytesIO(request.content), dtype=str, delimiter=process_config['delimiter'])
            ...
            try:
                ...
                cols_to_exclude = process_config['columns_exclude'] + [process_config['key_column']] 
                agg_cols_idx += [header.index(col) for col in header if col not in cols_to_exclude]
            except ValueError as error:
                self.logger.error(repr(error))
                context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                context.set_details(str(error))
                return ProcessDocumentReply()
            if len(agg_cols_idx) > process_config['max_number_of_cols']:
                msg = ("Number os columns to aggregate exceed!!"
                    f" Resizing from {len(agg_cols_idx)} to the first"
                    f" {process_config['max_number_of_cols']}")
                self.logger.warning(msg)
                context.set_details(msg)
                agg_cols_idx = agg_cols_idx[0:process_config['max_number_of_cols']]

            results += process_config['summary_header']
            line_idx = 0
            for idx in agg_cols_idx:
                unique_count = np.unique(document[:,idx]).shape[0]
                if (unique_count / document.shape[0]) <= process_config['max_unique_perc']:
                    companies_id = np.unique(document[:,key_column_idx])
                    for company_id in companies_id:
                        company_doc = document[document[:,key_column_idx] == company_id]
                        categories, count = np.unique(company_doc[:,idx], return_counts=True)
